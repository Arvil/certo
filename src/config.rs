...
pub fn load_root_certs() -> RootCertStore {
    ...
    match rustls_native_certs::load_native_certs() {
        Ok(certs) => {
            for cert in certs { root_store.add(&rustls::Certificate(cert.0)).unwrap(); }
            println!("Loaded native root certificate store.");
        }
        Err(_) => { // Fallback to webpki_roots
            println!("Failed to load native root certificate store, falling back to WebPKI...");
